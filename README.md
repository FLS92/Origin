...
chaque lundi, et commite les JSON mis à jour (`data/` et `docs/coffees-data.json`).
Ajouter le secret `ANTHROPIC_API_KEY` au repo GitHub pour activer la
génération de description.

## L'app (`docs/`)

`docs/index.html` est l'app Origin : elle charge `docs/coffees-data.json` via
...
```bash
python3 scripts/build_app_data.py   # lit data/*.json -> écrit docs/coffees-data.json
```
...
Le dossier s'appelle `docs/` (pas `app/`) parce que GitHub Pages en mode
"Deploy from a branch" ne propose que `/(root)` ou `/docs` comme source —
aucun nom de dossier personnalisé n'est accepté.
...
`main`, dossier `/docs`) une fois poussé sur GitHub. L'URL Pages obtenue sert
